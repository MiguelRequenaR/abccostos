...
import { TituloCard } from './titulo-card'
...
export default function TitulosCards() {
      ...
      {filteredData.length === 0 ? (
        ...
        <div className='grid grid-cols-1 gap-3'>
          {filteredData.map((titulo) => (
            <TituloCard key={titulo.codigo} titulo={titulo} />
          ))}
        </div>
      )}
    </div>
